...
echo "$($PSQL "
SELECT name FROM games JOIN teams ON games.winner_id=teams.team_id WHERE year=2014 AND round='Eighth-Final'
UNION
...
echo "$($PSQL "
SELECT DISTINCT(year), name FROM games JOIN teams ON games.winner_id=teams.team_id WHERE round='Final'
")"

echo -e "\nList of teams that start with 'Co':"
echo "$($PSQL "
SELECT name FROM games JOIN teams ON games.winner_id=teams.team_id WHERE name LIKE 'Co%'
")"
